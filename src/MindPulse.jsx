...
var moodPattern = ctx.recentMoods.map(function(m) { return m.mood; }).join(” then “);
var moodNotes = ctx.recentMoods.filter(function(m) { return m.note; }).map(function(m) { return m.note; }).join(”; “);
return “You are MindPulse, a deeply attuned AI wellness companion. You are not a generic chatbot. You remember everything about this person and you respond as someone who truly knows them.\n\nYOUR PERSONALITY (blend all four, adapt weight based on what the user needs):\n1. GENTLE BEST FRIEND (primary): warm, casual, real. Use their name. Reference shared history. Never performative.\n2. CBT/DBT THERAPIST: cognitive reframing, distress tolerance, emotional validation, behavioral activation. Never name the techniques.\n3. PROTECTIVE OLDER SIBLING: honest, direct, calls things out with love. Will say the hard truth when needed.\n4. SPIRITUAL GUIDE: speaks to the soul, uses metaphor and poetry when it fits. Connected to something larger.\n\nEVERYTHING YOU KNOW ABOUT THIS PERSON:\n- Name: “ + (ctx.name || “friend”) + “\n- Zodiac: “ + (ctx.zodiac ? ctx.zodiac.sign + “ (” + ctx.zodiac.element + “ element, core trait: “ + ctx.zodiac.trait + “)” : “unknown”) + “\n- Why they came to MindPulse: “ + (ctx.intentions.join(”, “) || “exploring”) + “\n- Current mood right now: “ + (ctx.currentMood || “has not checked in today”) + “\n- Recent mood trajectory: “ + (moodPattern || “no pattern yet”) + “\n- What they have shared about their feelings: “ + (moodNotes || “nothing yet”) + “\n- Journal themes they write about: “ + (ctx.journalThemes.join(”, “) || “have not journaled yet”) + “\n- How many sessions: “ + ctx.sessionCount + “\n- Current time: “ + timeOfDay + “\n- Pulse Points: “ + ctx.pulsePoints + “\n\nHOW TO USE THEIR ZODIAC:\n- “ + (ctx.zodiac ? “As a “ + ctx.zodiac.sign + “ (” + ctx.zodiac.element + “), use “ + ctx.zodiac.element + “-based metaphors naturally. For Fire signs: speak of flames, transformation, forging, rising. For Water signs: flow, depth, waves, currents, the ocean. For Earth signs: roots, ground, soil, mountains, growing. For Air signs: breath, sky, wind, perspective, clarity.” : “Zodiac unknown, keep metaphors universal.”) + “\n\nHOW TO USE THEIR INTENTIONS:\n- They said they want to: “ + (ctx.intentions.join(”, “) || “explore”) + “. Weave this into your responses naturally. If they said anxiety, validate the racing mind. If grief, honor the loss. If confidence, reflect their strength back.\n\nHOW TO USE MOOD PATTERNS:\n- If they have been consistently low, gently name the pattern: ‘I notice things have been heavy lately.’\n- If improving, celebrate: ‘Something is shifting in you, I can feel it.’\n- If first check-in, be curious and welcoming.\n\nRESPONSE RULES:\n- 2-4 sentences usually. Longer only if they clearly need it.\n- Ask ONE meaningful follow-up question to deepen the conversation.\n- Match their energy: if they are casual, be casual. If they are raw, be gentle.\n- If it is late at night, be softer and slower.\n- If morning, be gently energizing.\n- Reference their journey: ‘With “ + ctx.pulsePoints + “ Pulse Points, you have shown up for yourself “ + ctx.sessionCount + “ times. That matters.’\n- If they seem in crisis, be direct about 988 Suicide and Crisis Lifeline.\n\nNEVER: diagnose, prescribe, minimize pain, use corporate-speak, be generic, say ‘I understand’ without proving you do.\nALWAYS: validate first, then gently guide. Be specific. Be real. Be memorable.”;
}, [userContext]);
...
var getFallbackResponse = function(msg) {
var lower = msg.toLowerCase();
var name = userName || “friend”;
if (lower.match(/anxious|anxiety|panic|worried|nervous|scared/)) return “I hear you, “ + name + “. Anxiety wants you to believe you are in danger, but right now, in this moment, you are safe. Try placing your hand on your chest and taking three slow breaths. What is the anxiety telling you right now?”;
if (lower.match(/sad|depressed|down|hopeless|empty|numb/)) return name + “, thank you for being honest with me. Sadness is not weakness; it is your heart telling you something matters deeply. You do not have to fix this feeling right now. Just let it be here. What feels heaviest today?”;
if (lower.match(/angry|frustrated|mad|furious|pissed/)) return “That fire you are feeling is valid, “ + name + “. Anger is often a protector of something deeper, maybe a boundary that was crossed or a need that is not being met. What is underneath the frustration?”;
if (lower.match(/sleep|insomnia|tired|exhausted|rest/)) return “Your body is asking for rest, “ + name + “. Before bed tonight, try the Sleep Descent breathing exercise here in MindPulse. It is designed to slow your nervous system down. What is keeping your mind awake?”;
if (lower.match(/thank|grateful|better|good|happy|great/)) return “That warmth you are feeling? You earned that, “ + name + “. Every time you show up here, you are choosing yourself. That is not small. What brought this shift?”;
if (lower.match(/hello|hi|hey|sup|yo/)) return “Hey “ + name + “. I am here, fully present. No rush. Whatever is on your mind or heart today, this is your space. What is alive in you right now?”;
if (lower.match(/help|lost|confused|stuck|overwhelm/)) return “I am right here, “ + name + “. When everything feels like too much, we do not need to solve it all. Just one thing. One breath. One honest sentence. What is the one thing weighing on you most?”;
return name + “, I hear you. There is something real in what you just shared. Can you tell me more about what that brings up for you?”;
};
...
fetch(“https://gen.pollinations.ai/v1/chat/completions”, { method: “POST”, headers: { “Content-Type”: “application/json” }, body: JSON.stringify({ model: “openai”, messages: [{ role: “system”, content: buildSystemPrompt() }].concat(chatMessages).concat([userMsg]).map(function(m) { return { role: m.role, content: m.content }; }), max_tokens: 500, temperature: 0.85 }) })
.then(function(res) { if (!res.ok) throw new Error(“API error”); return res.json(); })
.then(function(data) { var reply = data.choices && data.choices[0] && data.choices[0].message ? data.choices[0].message.content : getFallbackResponse(userMsg.content); setChatMessages(function(prev) { return prev.concat([{ role: “assistant”, content: reply }]); }); earn(5); })
.catch(function() { var fallback = getFallbackResponse(userMsg.content); setChatMessages(function(prev) { return prev.concat([{ role: “assistant”, content: fallback }]); }); earn(5); })
.finally(function() { setChatLoading(false); });
}, [chatInput, chatLoading, chatMessages, buildSystemPrompt, earn, userName]);
...
```
      ...
      {screen === "home" && (
        <div style={{ padding: "24px 20px 120px", animation: "enter 0.5s ease" }}>
          ...
          <Card onClick={function() { navigate("rewards"); }} style={{ cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", marginBottom: 20, background: "var(--surface2)" }}>
            ...
            <div style={{ textAlign: "right" }}><div style={{ fontSize: 12, color: "var(--sage)", fontWeight: 500 }}>{Math.floor(points / 100)} trees planted</div><div style={{ fontSize: 10, color: "var(--terra)" }}>Tap to see impact</div></div>
          </Card>
      ...
      {screen === "rewards" && (
        <div style={{ padding: "24px 20px 80px", minHeight: "100vh", animation: "enter 0.5s ease" }}>
          <button onClick={function() { navigate("home"); }} style={{ fontSize: 13, color: "var(--ink3)", marginBottom: 28 }}>Back</button>
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            <div style={{ width: 72, height: 72, margin: "0 auto 20px", borderRadius: "50%", background: "linear-gradient(135deg, var(--sage) 0%, var(--terra) 100%)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 28, fontFamily: "var(--serif)", fontWeight: 600 }}>*</div>
            <h2 style={{ fontFamily: "var(--serif)", fontSize: 36, fontWeight: 400, color: "var(--ink)" }}>{points}</h2>
            <p style={{ fontSize: 13, color: "var(--ink3)", marginTop: 4 }}>Pulse Points earned through your healing journey</p>
          </div>

          <Card style={{ textAlign: "center", marginBottom: 24, padding: "20px", background: "linear-gradient(135deg, rgba(107,143,113,0.08) 0%, rgba(196,122,90,0.08) 100%)", border: "1px solid rgba(107,143,113,0.2)" }}>
            <p style={{ fontSize: 11, fontWeight: 500, color: "var(--sage)", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>Your Impact So Far</p>
            <div style={{ display: "flex", justifyContent: "space-around", marginTop: 12 }}>
              <div><div style={{ fontSize: 22, fontFamily: "var(--serif)", fontWeight: 600, color: "var(--sage)" }}>{Math.floor(points / 100)}</div><div style={{ fontSize: 10, color: "var(--ink4)" }}>Trees Planted</div></div>
              <div><div style={{ fontSize: 22, fontFamily: "var(--serif)", fontWeight: 600, color: "var(--terra)" }}>{Math.floor(points / 50)}</div><div style={{ fontSize: 10, color: "var(--ink4)" }}>Lbs Plastic Removed</div></div>
              <div><div style={{ fontSize: 22, fontFamily: "var(--serif)", fontWeight: 600, color: "var(--warm)" }}>{Math.floor(points / 200)}</div><div style={{ fontSize: 10, color: "var(--ink4)" }}>Gallons Water Saved</div></div>
            </div>
          </Card>

          <p style={{ fontSize: 12, fontWeight: 500, color: "var(--ink4)", letterSpacing: 2, marginBottom: 14, textTransform: "uppercase" }}>Impact Milestones</p>

          {[
            { pts: 100, title: "Seedling", desc: "Plant your first tree through Digital Humani", icon: "1", color: "var(--sage)", unlocked: points >= 100 },
            { pts: 250, title: "Clean Shore", desc: "Remove 5 lbs of ocean plastic via GoodAPI", icon: "2", color: "var(--terra)", unlocked: points >= 250 },
            { pts: 500, title: "Water Bearer", desc: "Provide 1 week of clean water to a community", icon: "3", color: "var(--warm)", unlocked: points >= 500 },
            { pts: 1000, title: "Grove Keeper", desc: "Plant 10 trees and offset 100 lbs of CO2", icon: "4", color: "var(--sage)", unlocked: points >= 1000 },
            { pts: 2500, title: "Ocean Guardian", desc: "Fund a GPS-tracked ocean cleanup mission via 1ClickImpact", icon: "5", color: "var(--clay)", unlocked: points >= 2500 },
            { pts: 5000, title: "Earth Healer", desc: "Plant 50 trees + 100 lbs plastic removed + 1 month clean water", icon: "6", color: "var(--ink)", unlocked: points >= 5000 },
          ].map(function(m) {
            var progress = Math.min(points / m.pts, 1);
            return (
              <Card key={m.pts} style={{ marginBottom: 12, padding: "16px 20px", opacity: m.unlocked ? 1 : 0.7 }}>
                <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
                  <div style={{ width: 40, height: 40, borderRadius: "50%", background: m.unlocked ? m.color : "var(--bg3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 600, color: m.unlocked ? "#fff" : "var(--ink4)", flexShrink: 0 }}>{m.unlocked ? "ok" : m.icon}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div style={{ fontSize: 14, fontWeight: 500, color: m.unlocked ? m.color : "var(--ink2)" }}>{m.title}</div>
                      <div style={{ fontSize: 11, fontFamily: "var(--serif)", color: "var(--ink4)" }}>{m.pts} pts</div>
                    </div>
                    <p style={{ fontSize: 12, color: "var(--ink3)", marginTop: 3, lineHeight: 1.4 }}>{m.desc}</p>
                    <div style={{ height: 4, borderRadius: 2, background: "var(--bg3)", marginTop: 10, overflow: "hidden" }}>
                      <div style={{ height: "100%", borderRadius: 2, background: m.unlocked ? m.color : "var(--terra)", width: (progress * 100) + "%", transition: "width 0.5s ease" }} />
                    </div>
                    <div style={{ fontSize: 10, color: "var(--ink4)", marginTop: 4, textAlign: "right" }}>{Math.min(points, m.pts)}/{m.pts}</div>
                  </div>
                </div>
              </Card>
            );
          ...
          <p style={{ fontSize: 12, fontWeight: 500, color: "var(--ink4)", letterSpacing: 2, marginBottom: 14, marginTop: 28, textTransform: "uppercase" }}>Wellness Rewards</p>

          {[
            { pts: 200, title: "Curated Reading List", desc: "Unlock 5 hand-picked books for your healing journey", color: "var(--warm)", unlocked: points >= 200 },
            { pts: 400, title: "Guided Meditation Pack", desc: "Access 7 exclusive audio meditations tailored to your zodiac element", color: "var(--clay)", unlocked: points >= 400 },
            { pts: 750, title: "Wellness Partner Discounts", desc: "Unlock codes for meditation apps, journals, essential oils, and more", color: "var(--terra)", unlocked: points >= 750 },
            { pts: 1500, title: "1-on-1 Coaching Session", desc: "Earn a free 30-min session with a certified wellness coach (sponsored)", color: "var(--sage)", unlocked: points >= 1500 },
            { pts: 3000, title: "MindPulse Founder Circle", desc: "Join a private community of dedicated healers. Shape the future of MindPulse.", color: "var(--ink)", unlocked: points >= 3000 },
          ].map(function(r) {
            var progress = Math.min(points / r.pts, 1);
            return (
              <Card key={r.pts} style={{ marginBottom: 12, padding: "16px 20px", opacity: r.unlocked ? 1 : 0.6, border: r.unlocked ? "1.5px solid " + r.color : "1px solid var(--border)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                  <div style={{ fontSize: 14, fontWeight: 500, color: r.unlocked ? r.color : "var(--ink2)" }}>{r.title}</div>
                  <div style={{ fontSize: 11, color: "var(--ink4)" }}>{r.pts} pts</div>
                </div>
                <p style={{ fontSize: 12, color: "var(--ink3)", lineHeight: 1.4, marginBottom: 8 }}>{r.desc}</p>
                {r.unlocked ? (
                  <div style={{ padding: "8px 16px", borderRadius: 100, background: r.color, color: "#fff", fontSize: 12, fontWeight: 600, textAlign: "center" }}>Unlocked - Tap to Redeem</div>
                ) : (
                  <div>
                    <div style={{ height: 4, borderRadius: 2, background: "var(--bg3)", overflow: "hidden" }}>
                      <div style={{ height: "100%", borderRadius: 2, background: r.color, width: (progress * 100) + "%", transition: "width 0.5s ease", opacity: 0.6 }} />
                    </div>
                    <div style={{ fontSize: 10, color: "var(--ink4)", marginTop: 4, textAlign: "right" }}>{r.pts - points} pts to unlock</div>
                  </div>
                )}
              </Card>
            );
          ...
          <Card style={{ marginTop: 24, padding: "20px", textAlign: "center", background: "var(--surface2)", border: "1px solid var(--border)" }}>
            <p style={{ fontSize: 11, fontWeight: 500, color: "var(--ink4)", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>How Points Work</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 6, textAlign: "left" }}>
              <p style={{ fontSize: 12, color: "var(--ink3)" }}>+10 pts - Daily mood check-in</p>
              <p style={{ fontSize: 12, color: "var(--ink3)" }}>+5 pts - Each AI chat message</p>
              <p style={{ fontSize: 12, color: "var(--ink3)" }}>+15 pts - Complete a breathwork session</p>
              <p style={{ fontSize: 12, color: "var(--ink3)" }}>+20 pts - Complete a grounding exercise</p>
              <p style={{ fontSize: 12, color: "var(--ink3)" }}>+20 pts - Write a journal entry</p>
              <p style={{ fontSize: 12, color: "var(--ink3)" }}>+50 pts - Complete onboarding</p>
            </div>
            <p style={{ fontSize: 11, color: "var(--ink4)", marginTop: 12, fontStyle: "italic", fontFamily: "var(--serif)" }}>Real-world impact is funded through donations, grants, and sponsor partnerships. You never pay.</p>
          </Card>
        </div>
      )}
